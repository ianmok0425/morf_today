...
class Mobile_Home_Category_Widget extends \Elementor\Widget_Base {
	...
	protected function render() {
		...
		// The Query
		$the_query = new WP_Query( array( 'posts_per_page' => 4, 'category_name' => $category ) );
		...
		if ( $the_query->have_posts() ) {
			...
			for ($i = 0; $i < count($post_ids); $i++) {
				...
				if ($image){
					$image_url = $image[0];
					$image1_html = '<img class="image1" src="' . $image_url . '" />';
				}else{
					$image1_html = '<img class="image1" src="" />';
				}
				$image = wp_get_attachment_image_src( get_post_thumbnail_id($post_ids[($i+1)%count($post_ids)]), 'full' );
				if ($image){
					$image_url = $image[0];
					$image2_html = '<img class="image2" src="' . $image_url . '" />';
				}else{
					$image2_html = '<img class="image2" src="" />';
				}
				$image = wp_get_attachment_image_src( get_post_thumbnail_id($post_ids[($i+2)%count($post_ids)]), 'full' );
				if ($image){
					$image_url = $image[0];
					$image3_html = '<img class="image3" src="' . $image_url . '" />';
				}else{
					$image3_html = '<img class="image3" src="" />';
				}
				$image_html = 
				...
				$title_html = '<div class="title">' . get_the_title($post_id) . '</div>';
				$excerpt_html = '<div class="excerpt">' . get_the_excerpt($post_id) . '</div>';
				$link = get_post_permalink($post_id);
				$readmore_html = '<div><a class="read-more" href="' . $link . '">' . 'Read more...' . '</a></div>';

				$row3 = '<div class="excerpt-readmore">' . $title_html . $excerpt_html . $readmore_html . '</div>';
